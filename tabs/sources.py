...
def register_sources_tab_callbacks(app):
    ...
    def update_sources_tab(n_clicks, active_tab, lang_val, db_val, source_type, start_date, end_date):
        ...
        try:
            ...
            # Create content with actual data summaries - handle both DataFrame and dict returns
            def safe_len(data):
                """Safely get length of data whether it's DataFrame, dict, list, or None"""
                if data is None:
                    return 0
                if hasattr(data, 'empty') and data.empty:  # DataFrame
                    return 0
            ...
            documents_summary = html.Div([
                ...
                html.P(f"Total documents: {documents_data.get('total_documents', 0) if isinstance(documents_data, dict) else safe_len(documents_data):,}"),
                html.P("Document counts by language, database, and date range")
            ])
            
            chunks_summary = html.Div([
                html.H5("Text Chunks Overview"), 
                html.P(f"Total chunks: {chunks_data.get('total_chunks', 0) if isinstance(chunks_data, dict) else safe_len(chunks_data):,}"),
                html.P("Text chunks extracted from documents for analysis")
            ])
            
            taxonomy_summary = html.Div([
                html.H5("Taxonomy Elements Overview"),
                html.P(f"Total taxonomy elements: {taxonomy_data.get('total_combinations', 0) if isinstance(taxonomy_data, dict) else safe_len(taxonomy_data):,}"),
                html.P("Hierarchical taxonomy elements assigned to chunks")
            ])
            
            keywords_summary = html.Div([
                html.H5("Keywords Overview"),
                html.P(f"Total unique keywords: {keywords_data.get('unique_keywords', 0) if keywords_data and isinstance(keywords_data, dict) else 0:,}"),
                html.P("Extracted keywords and their frequency distribution")
            ])
            
            entities_summary = html.Div([
                html.H5("Named Entities Overview"),
                html.P(f"Total unique entities: {named_entities_data.get('unique_entities', 0) if named_entities_data and isinstance(named_entities_data, dict) else 0:,}"),
            ...
            return stats_html, updated_tabs_content, {'display': 'none'}, {'display': 'block'}
